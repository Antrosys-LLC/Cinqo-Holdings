...
export default function Credibility() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      items.forEach((item, i) => {
        ...
        tl.fromTo(
          ...
          .fromTo(
            wipeEl,
            { scaleX: 1 },
            { scaleX: 0, duration: 0.55, ease: "power4.inOut", transformOrigin: "right" },
            0.15,
          )
      ...
      if (videoRef.current && window.matchMedia("(prefers-reduced-motion: no-preference)").matches) {
  ...
  useEffect(() => {
    ...
    const setSpotX = spotlight ? gsap.quickTo(spotlight, "x", { duration: 0.45, ease: "power3.out" }) : null;
    const setSpotY = spotlight ? gsap.quickTo(spotlight, "y", { duration: 0.45, ease: "power3.out" }) : null;
    const setVideoX = video ? gsap.quickTo(video, "x", { duration: 1.1, ease: "power3.out" }) : null;
    const setVideoY = video ? gsap.quickTo(video, "y", { duration: 1.1, ease: "power3.out" }) : null;
    ...
    const onSectionEnter = () => gsap.to(spotlight, { opacity: 1, duration: 0.4 });
    ...
    const itemCleanups = items.map((item) => {
      ...
      const setRotateX = gsap.quickTo(item, "rotateX", { duration: 0.5, ease: "power3.out" });
      const setRotateY = gsap.quickTo(item, "rotateY", { duration: 0.5, ease: "power3.out" });
      const setLift = gsap.quickTo(item, "y", { duration: 0.5, ease: "power3.out" });
      ...
      const onEnter = () => {
        gsap.to(numberEl, { color: "#ea5a42", duration: 0.3, ease: "power2.out" });
        gsap.to(underlineEl, { scaleX: 1.4, duration: 0.4, ease: "power2.out" });
      };
      const onLeave = () => {
        setRotateX(0);
        setRotateY(0);
        setLift(0);
        gsap.to(numberEl, { color: "#ffffff", duration: 0.4, ease: "power2.out" });
  ...
  >
    ...
    />
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle, rgba(234,90,66,0.35), rgba(234,90,66,0) 70%)",
          filter: "blur(10px)",
        }}
      />
